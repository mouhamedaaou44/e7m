module.exports = async function(Stable_Version) {
    ...
    const { body } = await got('https://raw.githubusercontent.com/TanzidAhmadHamim/fca-h4m1m-x2/main/package.json');
    ...
    const LocalVersion = require('../../package.json').version;
        if (Number(LocalVersion.replace(/\./g,"")) < Number(json.version.replace(/\./g,"")) && global.Fca.Require.FastConfig.Stable_Version.Accept == false || Stable_Version && Number(LocalVersion.replace(/\./g,"")) != Number(Stable_Version.replace(/\./g,""))) {
            ...
            log.warn("[ FCA-X2 ] •","New Version, Ready to Update: " + LocalVersion + " -> " + Version);    
            await new Promise(resolve => setTimeout(resolve, 3000));
            try {
                execSync(`npm install fca-h4m1m-x2@${Version}`, { stdio: 'inherit' });
                ...
                try {
                    ...
                    try {
                        console.log(err);
                        log.warn("[ FCA-X2 ] •","Update Failed, Trying to clean Database() cache...");
                        await new Promise(resolve => setTimeout(resolve, 3000));
                        ...
                        await new Promise(resolve => setTimeout(resolve, 3000));
                        log.warn("[ FCA-X2 ] •","Please contact to owner about update failed and screentshot error log at fb.com/Lazic.Kanzu");
                        await new Promise(resolve => setTimeout(resolve, 3000));
                        process.exit(1);
                    }
                }
