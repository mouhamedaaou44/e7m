...
module.exports.run = async ({ args, api, event, Currencies, client }) => {
   ...
   const moment = require("moment-timezone");
    var timeNow = moment.tz("Africa/Tunis").format("HH:mm:ss")
   if (args.length == 0) return api.sendMessage(`
   ❋・────━【📝】━────・❋
⌯︙احسبي  رسائلي  ¦✗¦← تحسب رسائلك ورسائل الشخص اللي تسويله تاغ 

⌯︙احسبي التشغيل ¦✗¦← تحسب وقت تشغيل البوت

⌯︙احسبي المجموعات¦✗¦←تحسب عدد المجموعات اللي فيها البوت

⌯︙احسبي الكل ¦✗¦← تحسب الأعضاء الأكثر تفاعلا بالمجموعة

⌯︙احسبي الإصابات ¦✗¦←تحسب عدد اصابات كوفيد-19 في العالم

🕞︙الوقت الحالي حسب توقيت تونس:
  ｢〆｣➟ 「${timeNow}」`, event.threadID, event.messageID);
    var arr = [];
  ...
  return api.sendMessage(
    ...
    event.threadID, event.messageID
      ...
      if (event.senderID != 100092631887370) return api.sendMessage(`ماعدك صلاحية وردة`, event.threadID, event.messageID)
            let number = [];
